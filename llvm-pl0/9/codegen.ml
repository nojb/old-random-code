...
and reference_load c m b env refn =
  ...
  | Array (base, idx) ->
    begin
      ...
      let elmtype = Types.to_lltype c
        (Types.array_element_type base.reference_type)
      in
      let baseval = build_pointercast baseval
        (pointer_type elmtype) "cast" b
      in
      let idx = expression c m b env idx in
      let gep = build_gep baseval [|idx|] "tmp" b in
        build_load gep "tmp" b
    end
