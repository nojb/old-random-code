...
and reference lvl env add_free_var refn =
  ...
  | Ast.Var n ->
    begin
      ...
      | Primitive _ ->
          failwith "[semant] can't use prim in expression"
      | Type _ ->
  ...
  | Ast.Field (base, field_name) ->
    begin
      let base = reference lvl env add_free_var base in
        if not (Types.is_record base.reference_type) then
          failwith "not a record"
        else
          begin
            try let field = Types.record_field base.reference_type field_name in
              {
                reference_desc = Field (base, field);
                reference_type = field.Types.field_type;
              }
            with Not_found -> failwith "record field not found"
          end
    end
  ...
  | Ast.Call (n, args) ->
    begin
      ...
      | Primitive prim ->
          ...
          CallPrimitive (prim, List.map (expression lvl env add_free_var) args)
      | Variable _ ->
...
let base_env () =
  ...
  let add_prim name impl types =
    Env.add env name (Primitive
      {
        primitive_name = name;
        primitive_implementation_name = impl;
        primitive_parameter_types = types;
      })
  in
    Env.add env "INTEGER" (Type Types.integer);
    Env.add env "BOOLEAN" (Type Types.boolean);
    add_prim "WriteInt" "_WriteInt" [Types.integer];
    add_prim "WriteHex" "_WriteHex" [Types.integer];
    add_prim "WriteLn" "_WriteLn" [];
    env
          ...
          let rec loop idx = function
            | (n,t) :: rest ->
              begin
                {
                  Types.field_name = n;
                  Types.field_type = lookup_type env t;
                  Types.field_index = idx;
                } :: loop (idx+1) rest
              end
            | [] -> []
          in Types.Record (loop 0 fields)
      }
...
let rec block lvl env add_free_var blk =
  ...
  let add_type (n, t) =
    Env.add env n (Type (lookup_type env t))
  in
  ...
  in
    ...
    (* Add the block's types *)

    List.iter add_type blk.Ast.block_types;
