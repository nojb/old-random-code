    ...
    begin
      ...
      | Variable var ->
        begin
          ...
          {
            expression_desc = Ref (Var var);
            expression_type = var.variable_type;
          }
        end
      | Parameter param ->
        begin
          ...
          {
            expression_desc = Ref (Param param);
            expression_type = param.parameter_type;
          }
        end
      | Constant _ -> failwith "[semant] not implemented"
      | Procedure _ ->
...
let expression_with_type lvl env add_free_var expr t =
    ...
    begin
      ...
      | Variable var ->
        begin
          ...
          let e = expression lvl env add_free_var e in
            if not (Types.equal e.expression_type var.variable_type) then
              failwith "incompatible types in assignment"
            else
              Assign (Var var, e)
        end
      | Parameter param ->
        begin
          ...
          let e = expression_with_type lvl env
            add_free_var e param.parameter_type
          in
            Assign (Param param, e)
        end
      | Constant _ ->
      ...
      | Type _ ->
          failwith "[semant] can't assign to type"
      with Not_found ->
        failwith "[semant] variable not found"
    end
  | Ast.Call (n, args) ->
